...
export default function CinemaScroll({ projects }: CinemaScrollProps) {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      const cardW = Math.min(vw * 0.72, 900)
      const cardH = Math.min(vh * 0.72, 640)

      // Card starts off-screen below (already position:fixed via CSS)
      gsap.set(card, {
        y: vh + 200,
        width: cardW,
        height: cardH,
      ...
      // All card sections start hidden
      ...
      // 2.5→4.5 : card expands to full screen
      tl.to(card, {
        width: vw,
        height: vh,
        borderRadius: 0,
      ...
      // CTA — card pulls back to rounded card
  ...
  return (
    <>
      ...
      {/* ── Physical card — fixed, GSAP moves it ─────────────────────────── */}
      <div
        ref={cardRef}
        className="fixed z-20 overflow-hidden"
        style={{
          /* starts invisible — GSAP sets opacity:1 and y:vh+200 on mount */
          opacity: 0,
